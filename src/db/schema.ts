...
export const roleTable = sqliteTable('roles_table', {
    roleId: int().primaryKey({ autoIncrement: true }),
    roleName: text().notNull().unique(),
})
...
export const manufacturerRelations = relations(manufacturerTable, ({one, many}) => ({
    carTable : many(carTable)
}));
...
export const transmissionRelations = relations(transmissionTable, ({many}) => ({
    carTable: many(carTable),
}));
...
export const insuranceProviderRelations = relations(insuranceProviderTable, ({many}) => ({
...
export const insuranceTypeRelations = relations(insuranceTypeTable, ({many}) => ({
    insuranceTable: many(insuranceTable),
}))

export const paymentStatusRelations = relations(paymentStatusTable, ({many}) => ({
    invoiceTable: many(invoiceTable),
}))

export const paymentMethodRelations = relations(paymentMethodTable, ({many}) => ({
    invoiceTable: many(invoiceTable),
}))
...
export const reservationStatusRelations = relations(reservationStatusTable, ({many}) => ({
